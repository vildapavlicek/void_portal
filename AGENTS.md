...
Gameplay logic must follow this execution flow, enforced via the `VoidGameStage` SystemSets in `common`:

1.  **Phase 0: Frame Start (`VoidGameStage::FrameStart`)**
    - Initial processing required before decisions (e.g., time updates, sensor readings).
...
3.  **Phase 2: Execution/Action (`VoidGameStage::Actions`)**
    - Systems resolve `Intent` into **Messages** or intermediate states (e.g., `melee_execution_system` emits `DamageMessage`).
4.  **Phase 3: Application/Effect (`VoidGameStage::Effect`)**
...
- **`assets`**: Asset loading and management (`VoidAssetsPlugin`).
- **`common`**: Shared types, `GameState`, `VoidGameStage`, and global **Messages**.
...
- **`monster_factory`**: Spawning infrastructure and factory logic.
- **`player_npcs`**: Player ally logic (Soldiers, Heroes).
- **`player_npcs_ui`**: UI specific to player NPCs (e.g., Soldier stats).
- **`portal`**: Core portal mechanics, upgrades, spawning logic.
- **`ui`**: General User Interface systems (Wallet, Portal Panel).
- **`vfx`**: Visual effects and floating text system.
